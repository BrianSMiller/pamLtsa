...
function output = timeMean(specgram, signalTime, timeVector, timeWindow)
len = length(timeVector);
siz = size(specgram);
output = nan(siz(1), len);

for count = 1:len
    t = timeVector(count);
    ix = find((abs(t-signalTime)*86400 <= timeWindow));
    if ~isempty(ix) 
        output(:,count) = mean(specgram(:,ix),2);
    end
end
