% function output = timeAvg(signal, signalTime, timeVector, timeWindow)
...
function output = timeStd(signal, signalTime, timeVector, timeWindow)
len = length(timeVector);
output = zeros(size(timeVector));

for count = 1:len
    t = timeVector(count);
    ix = find((abs(t-signalTime)*86400 <= timeWindow) & ~isnan(signal));
    output(count) = std(signal(ix));
end
output = output(:);